...
class basics (commands.Cog) :
    ...
    async def pb (self, ctx) :
        with open('files/GDriveLink.txt', 'r', encoding = 'utf-8') as GDFile :
...
with open('files/GuildID.txt', 'r') as GuildFile :
        GuildID = GuildFile.read()


async def setup(bot):
    await bot.add_cog(basics(bot), guilds = [discord.Object(id = GuildID)])
